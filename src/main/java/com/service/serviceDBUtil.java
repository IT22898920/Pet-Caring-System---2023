...
public class serviceDBUtil {
	...
	public static boolean insertservicedetails(String name, String petNum, String model, String date, String email) {
	//PARAMETERS INSIDE ----> Data passed from the servlet is caught here.
		...
		try {
		    ...
		    String sql = "insert into servicedetails values (0,'"+name+"','"+petNum+"','"+model+"','"+date+"','"+email+"')";
	...
	public static List<service> getRequestDetails(String username) {
	    	...
	    	try {
	    		...
	    		while(rs.next()) {
	    			...
	    			service s = new service(id,serviceName,petNo,model,appointmentDate,email);
	    			service.add(s);
	    		}
	...
	public static boolean updateRequest(String serviceID, String serviceName, String petNum, String model, String date, String email) {
    	int ID = Integer.parseInt(serviceID);
    	...
    	try {
    		...
    		String sql = "update servicedetails set ServiceName='"+serviceName+"',PetNum='"+petNum+"',Model='"+model+"',AppointmentDate='"+date+"'"
    				+ " where ServiceID="+ID;
